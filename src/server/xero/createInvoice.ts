...
export const createXeroInvoice = async (input: CreateInvoiceOptions) => {
  ...
  return Sentry.startSpan(
    ...
    async () => {
      ...
      const createInvoiceResponse = await Sentry.startSpan(
        {
          name: "xero.accountingApi.createInvoices",
          op: "http.client",
          attributes: { orderId },
        },
        async () =>
          xero.accountingApi.createInvoices(activeTenantId, {
      ...
      await Sentry.startSpan(
        {
          name: "xero.accountingApi.createPayments",
          op: "http.client",
          attributes: { orderId, xeroInvoiceId },
        },
        async () => xero.accountingApi.createPayments(activeTenantId, payment),
      );

      const order = await Sentry.startSpan(
        {
          name: "xero.invoice.updateOrder",
          op: "db",
          attributes: { orderId, xeroInvoiceId },
        },
        async () =>
      ...
      void Sentry.startSpan(
        {
          name: "xero.accountingApi.emailInvoice",
          op: "http.client",
          attributes: { orderId, xeroInvoiceId },
        },
        async () =>
...
export const createInvoiceFromStripeEvent = async (
  ...
  return Sentry.startSpan(
    {
      name: "xero.invoice.fromStripeEvent",
      op: "function",
      attributes: {
    ...
    async () => {
      try {
        ...
        const shippingOption = await Sentry.startSpan(
          {
            name: "stripe.shippingRates.retrieve",
            op: "http.client",
            attributes: {
              orderId: event.metadata!.orderId!,
          ...
          async () => stripe.shippingRates.retrieve(order!.shippingRateId!),
        );
